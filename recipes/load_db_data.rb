...
bash 'run-osm2pgsql-to-import-data' do
  code <<-EOH
    ...
    /usr/local/bin/osm2pgsql -U #{node[:pg][:user]} -d #{node[:pg][:dbname]} -H localhost --slim --hstore --cache 2048 --merc --prefix planet_osm --style #{node[:path]}/src/vector-datasource/osm2pgsql.style --verbose --number-processes 1 #{node[:path]}/var/pbf/#{node[:pbf][:name]} 2>&1 >#{node[:path]}/var/log/osm2pgsql-initial-import.log
  EOH
end
...
bash 'import vector-datasource data' do
  ...
  EOH
  cwd "#{node[:path]}/src/vector-datasource/data"
end
